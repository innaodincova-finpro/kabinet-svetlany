...
s=s.replace("const VERSION = '31 августа 2026';", "const VERSION = '8 сентября 2026 · помощь 1.3';")
...
s=s.replace('function viewHelp() {', 'function viewHelpContent() {')
s=s.replace("fmtDate(lastBackup()) + ' ('", "helpDateTime(helpBackupTime()) + ' ('")
s=s.replace("'Версия кабинета: ' + VERSION", "'Версия кабинета: ' + VERSION + ' · сборка ' + helpDateTime('2026-09-08T06:07:31Z')")
s=s.replace("'Хранилище браузера: ' +", "'Последнее сохранение в браузере: ' + helpDateTime(helpSavedTime()) + '. Хранилище браузера: ' +")
s=s.replace('dropOldDrafts();\nstartTick();', (root/'source/help.js').read_text()+'\ndropOldDrafts();\nstartTick();')
s=s.replace('function save(label) {','function legacySave(label) {')
s=s.replace('dropOldDrafts();\nstartTick();', (root/'source/pending-save.js').read_text()+'\ndropOldDrafts();\nstartTick();')
s=s.replace('safeRestoreAuto();','safeRestoreAuto();\npendingRecover();')
s=s.replace('function takeOver(auto) {', 'function takeOver(auto) {\n  if(pendingSave){pendingShow();return;}')
s=s.replace("          m.ch = m.s === 'был';\n          renderAfter();", "          m.ch = m.s === 'был';\n          renderAfter();\n          body.dispatchEvent(new Event('input',{bubbles:true}));")
s=s.replace('async function autoSaveNow(loud) {','async function autoSaveNow(loud) {\n if(pendingSave)return false;')
# User-facing event dates; existing records and storage keys are unchanged.
s=s.replace("at: nowHM()", "at: new Date().toISOString()").replace("at:nowHM()", "at:new Date().toISOString()")
...
exec((root/'source/lesson-time.py').read_text())
exec((root/'source/upcoming-route.py').read_text())
...
sw=sw.replace('svetlana-homework-20260907-3','svetlana-schedule-20260913-1').replace('svetlana-schedule-20260913-1','svetlana-lessontime-20260916-1')
sw=sw.replace('svetlana-lessontime-20260916-1','svetlana-route-20260916-1')
