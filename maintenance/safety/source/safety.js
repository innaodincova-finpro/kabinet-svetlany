...
async function safeRecoveryList() {
  ...
  const seen=new Set();for(const item of candidates){
    if(item.file){body.append(el('div',{class:'card'},el('p',{},item.reason+' · '+item.at+' · '+fmtSize(item.file.size)),el('button',{class:'btn',onclick:()=>{dlg.close();safeImportFile(item.file)}},'Выбрать копию')));continue;}
    if(seen.has(item.text))continue;seen.add(item.text);try{const d=JSON.parse(item.text);safePrepare(d);body.append(el('div',{class:'card'},el('p',{},item.reason+' · '+(item.at||d.updatedAt||'без даты')+' · '+d.students.length+' учеников, '+d.lessons.length+' занятий'),el('button',{class:'btn',onclick:()=>{dlg.close();safeImportFile(new Blob([item.text]))}},'Выбрать копию')))}catch{}
  }
  if(!seen.size)body.append(el('p',{},'В браузере не найдены копии. Можно загрузить файл из OneDrive.'));
}
