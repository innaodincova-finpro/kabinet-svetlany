...
function boot(html){
 ...
 let failPrimary=false,failAll=false; let raw={};const storage={getItem:k=>Object.hasOwn(raw,k)?raw[k]:null,setItem:(k,v)=>{if(failAll || (failPrimary && k==='tochka-resheniya-v2'))throw Error('quota');raw[k]=String(v)},removeItem:k=>delete raw[k],key:i=>Object.keys(raw)[i]??null,get length(){return Object.keys(raw).length}};
