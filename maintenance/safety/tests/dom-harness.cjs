...
function boot(html, initial={}){
 ...
 let failPrimary=false,failAll=false; let raw={...initial.storage};const storage={getItem:k=>Object.hasOwn(raw,k)?raw[k]:null,setItem:(k,v)=>{if(failAll || (failPrimary && k==='tochka-resheniya-v2'))throw Error('quota');raw[k]=String(v)},removeItem:k=>delete raw[k],key:i=>Object.keys(raw)[i]??null,get length(){return Object.keys(raw).length}};
 ...
 names.push('upcomingTopics','viewPersonCard','standingCard','editStandingSlot','standingPlan','spreadSlots','pendingRecover','pendingRetry','takeOver','undo','editMaterial','prevHomework','passedTopics','homeworkFor','homeworkCheckFor','homeworkSolutionView','homeworkSolutionEditor','openLessonCard','reportBlocks','weekGrid','icsText','lessonProblems','brokenLessonsBlock','newLesson','overlaps');
 const expose=`window.testAPI={${names.map(n=>n+':(typeof '+n+'===\"undefined\"?undefined:'+n+')').join(',')},setPerson:key=>{ui.personId=key},getState:()=>S,setState:d=>{S=d;lastSnap=JSON.stringify(S);dataUnreadable=false;readOnlyTab=false},setFileReader:fn=>testFileReader=fn,getFlags:()=>({dataUnreadable,readOnlyTab,safeBusy,autoFailed,safeStatus,safeLastSnapshot}),setAuto:h=>{autoHandle=h;autoOn=true;autoFailed=false;safeLastSnapshot=null},setFlags:f=>{if('readOnlyTab'in f)readOnlyTab=f.readOnlyTab;if('dataUnreadable'in f)dataUnreadable=f.dataUnreadable},setStorageFailure:failStorage,setAllStorageFailure:failAllStorage,getRaw:()=>localStorage.getItem(KEY),putRaw:v=>localStorage.setItem(KEY,v)};`;
