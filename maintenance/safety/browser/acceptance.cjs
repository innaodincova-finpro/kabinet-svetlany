...
(async()=>{
 ...
 for(const spec of [{name:'chromium-desktop',engine:chromium,width:1366,height:900},{name:'chromium-mobile-390',engine:chromium,width:390,height:844},{name:'chromium-mobile-320',engine:chromium,width:320,height:740},{name:'webkit-mobile-390',engine:webkit,width:390,height:844}]){
  ...
  try{
   ...
   await help.nth(0).locator(':scope > summary').click();
   if(await menu.isVisible())await menu.click();
   ...
   await help.nth(4).locator(':scope > summary').click();
   ...
   results.push({name:spec.name,passed:true,scenarios:['save-reload-personal-text-and-PDF','next-lesson-personal-answer','PDF-canvas-render','separate-checks-and-reports','narrow-form-controls','incomplete-lesson-time']});console.log('PASS',spec.name);
